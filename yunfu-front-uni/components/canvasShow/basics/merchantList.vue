...
import API from '@/config/api'; // 对齐参考页面的 API 使用方式
...
export default {
    ...
    data() {
        ...
        return {
            ...
            size: initParams.pageSize || 10, // 本地仍叫 size，用于内部计算；对外请求用 pageSize
            total: 0,
            loading: false,
            finished: false,
    ...
    methods: {
        // 和参考页保持：内部维护 sort 字符串，对请求映射为 type
        sortKeyToType(key) {
            if (key === 'distance') return 1;
            if (key === 'sales') return 3;
            return 2; // recommend 或默认
        },
        onChangeSort(key) {
        ...
        async fetchList(reset = false) {
            ...
            this.loading = true;
            try {
                // 参考页风格：request(API.xxx, params, 'GET')
                const baseParams = {
                    page: this.page,
                    pageSize: this.size,
                    // 兼容后端：既传 sort（字符串），也传 type（数字）
                    sort: this.curSort,
