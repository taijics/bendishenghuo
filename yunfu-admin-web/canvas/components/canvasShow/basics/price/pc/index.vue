<template>
  <div class="product-list" :class="'terminal' + terminal">
    <div v-if="componentContent.arrangeType == '横向滑动'" class="picListWarp">
      <div
        v-if="
          productData.composeProducts && productData.composeProducts.length > 0
        "
        class="picList"
      >
            ...
            <div class="a-link" @click="jumpProductDetail(item)">
              ...
              <div class="text">
                <div class="discount-text">
                  <span>任选{{ productData.rules[0].number }}件{{
                    productData.rules[0].price
                  }}元</span>
                </div>
                ...
                <button class="btn-cart" @click="addCart(item.id)">
                  加入购物车
                </button>
              </div>
            </div>
    ...
    <div v-else class="picList">
      <ul class="clearfix" :class="'imgTextNum' + componentContent.productNum">
        <li
          v-for="(item, index) in productData.composeProducts.slice(
            0,
...
const { terminal, shopId, componentContent } = toRefs(props);
  ...
  .picList {
    ...
    .a-link {
      ...
      .text {
        ...
        //height: 180px;
        .discount-text {
          background: url("../../../static/images/price/bg-discount.png")
            no-repeat;
          width: 100%;
          height: 47px;
          font-size: 14px;
