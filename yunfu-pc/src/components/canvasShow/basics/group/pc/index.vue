<template>
  <div class="product-list" :class="'terminal'+terminal">
    <div class="picListWarp" v-if="componentContent.arrangeType == '横向滑动'">
      <div class="picList" v-if="productData && productData.products">
        <div class="group-button-prev"></div>
        <div class="group-button-next"></div>
    ...
    <div v-else class="picList" >
      ...
      >
        <li class="item"
          v-for="(item,index) in productData.products.slice(0, componentContent.productRowNum * componentContent.productNum)"
          :key="index"
        >
          <div class="a-link" @click="jumpProductDetail(item)">
            <div class="itemImgBox">
...
.product-list{
  ...
  .picList{
    ...
    max-width: 100%;
    margin: 0 auto;
    .group-button-prev,.group-button-next{
      width: 50px;
      height: 50px;
      position: absolute;
      cursor:pointer;
