    ...
    <div class="centBox">
      ...
      <div class="mFormBox">
        ...
        <div class="subjectInfo" v-show="active === 2">
          <div class="form">
            <div class="inputItem">
              <div class="labelTit"><label><i>*</i>企业名称：</label></div>
              <input type="text" v-model="personalForm.enterpriseName" placeholder="请输入企业名称">
            </div>
            <p class="describe">需与当地政府颁发的商业许可证书或企业注册证上的企业名称完全一致，信息审核审核
              成功后，企业名称不可修改</p>
            <div class="inputItem">
              <div class="labelTit"><label><i>*</i>社会信用代码：</label></div>
              <input type="text" v-model="personalForm.enterpriseCode" placeholder="请输入社会信用代码">
            </div>
            <div class="inputItem arrow">
              <div class="labelTit"><label><i>*</i>注册地址：</label></div>
              <input @click="showRegionPopup" @focus="noBomBox" v-model="personalForm.enterpriseRegion" type="text" placeholder="请选择注册地址">
              <span class="iconfont">&#xe6ab;</span>
              <input type="text" v-model="personalForm.enterpriseAdress" placeholder="请输入详细地址">
            </div>
            <div class="inputItem arrow">
              <div class="labelTit"><label><i>*</i>营业期限：</label></div>
              <input type="text" @focus="noBomBox" v-model="personalForm.enterpriseStartTime" placeholder="请选择营业开始时间" @click="selectStartDateBtn">
              <span class="iconfont">&#xe6ab;</span>
            </div>
            <div class="inputItem arrowTime">
              <input v-model="personalForm.enterpriseEndTime" @focus="noBomBox" type="text" placeholder="请选择营业截止时间" @click="selectEndDateBtn">
              <span class="iconfont">&#xe6ab;</span>
            </div>
            <p class="describe">
        ...
        <div class="subjectInfo" v-show="active === 3">
          <div class="form">
            <div class="inputItem">
              ...
              <input v-model="personalForm.enterpriseOperator" type="text" placeholder="请输入经营者姓名">
            </div>
            <div class="inputItem arrow">
            ...
            <div class="inputItem">
              ...
              <input v-model="personalForm.enterpriseIdCard" type="text" placeholder="请输入居民身份证号">
            </div>
            <div class="inputItem arrow">
              <div class="labelTit"><label><i>*</i>身份证有效期：</label></div>
              <input v-model="personalForm.enterpriseCardStartTime" type="text" @focus="noBomBox" placeholder="请选择身份证开始时间" @click="selectIdStartDateBtn">
              <span class="iconfont">&#xe6ab;</span>
            </div>
            <div class="inputItem arrowTime">
              <input v-model="personalForm.enterpriseCardEndTime" @focus="noBomBox" type="text" placeholder="请选择身份证有效期结束时间" @click="selectIdEndDateBtn">
              <span class="iconfont">&#xe6ab;</span>
            </div>
            <div class="inputItem">
...
export default {
  ...
  methods: {
    ...
    selectRegionArea (address) {
      this.personalForm.enterpriseRegion = `${address[0].name}-${address[1].name}-${address[2].name}`
    ...
    onConfirm (type) {
      ...
      this.personalForm.enterpriseCardType = type.value
      this.idTypeShow = false
    },
    onCancel () {
      this.personalForm.enterpriseCardType = ''
      this.idTypeShow = false
    },
    // 选择开始时间
    selectStartTime (value) {
      this.personalForm.enterpriseStartTime = this.dateTime(value)
    ...
    // 选择截止时间
    selectEndTime (value) {
      this.personalForm.enterpriseEndTime = this.dateTime(value)
    ...
    selectIdCardStartTime (value) {
      this.personalForm.enterpriseCardStartTime = this.dateTime(value)
    ...
    selectIdCardEndTime (value) {
      this.personalForm.enterpriseCardEndTime = this.dateTime(value)
      ...
      self.sendReq(params, (res) => {
        if (res.code === '200') {
          if (type === 'license') {
            ...
            self.personalForm.enterpriseCardPositive = res.data.url
          } else if (type === 'back') {
            self.personalForm.enterpriseCardSide = res.data.url
          }
        } else {
    ...
    submitForm () {
      ...
      if (self.personalForm.enterpriseName === '') {
        Toast.fail('请输入商户名称')
        return false
      }
      if (self.personalForm.enterpriseCode === '') {
        Toast.fail('请输入社会信用代码')
        return false
      }
      if (self.personalForm.enterpriseRegion === '') {
        Toast.fail('请输入注册地址')
        return false
      }
      if (self.personalForm.enterpriseAdress === '') {
        Toast.fail('请输入注册详情地址')
        return false
      }
      if (self.personalForm.enterpriseStartTime === '') {
        Toast.fail('请选择营业开始时间')
        return false
      }
      if (self.personalForm.enterpriseEndTime === '') {
        Toast.fail('请选择营业截止时间')
        return false
      }
      if (self.personalForm.enterpriseLicense === '') {
        Toast.fail('请上传营业执照')
        return false
      }
      if (self.personalForm.enterpriseOperator === '') {
        Toast.fail('请输入经营者姓名')
        return false
      }
      if (self.personalForm.enterpriseCardType === '') {
        Toast.fail('请选择证件类型')
        return false
      }
      if (self.personalForm.enterpriseIdCard === '') {
        Toast.fail('请输入居民身份证号')
        return false
      } else if (!idReg.test(self.personalForm.enterpriseIdCard)) {
        Toast.fail('身份证号码格式错误')
        return false
      }
      if (self.personalForm.enterpriseCardStartTime === '') {
        Toast.fail('请选择身份证开始时间')
        return false
      }
      if (self.personalForm.enterpriseCardEndTime === '') {
        Toast.fail('请选择身份证有效期结束时间')
        return false
      }
      if (self.personalForm.enterpriseCardPositive === '') {
        Toast.fail('请上传身份证正面照')
        return false
      }
      if (self.personalForm.enterpriseCardSide === '') {
