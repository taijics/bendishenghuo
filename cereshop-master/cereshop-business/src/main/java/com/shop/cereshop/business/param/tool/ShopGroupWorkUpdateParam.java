 ...
 */
@Data
@ApiModel(value = "ShopGroupWorkUpdateParam", description = "修改拼团活动请求")
public class ShopGroupWorkUpdateParam implements Serializable {
    /**
     * 店铺拼团活动id
     */
    @ApiModelProperty(value = "店铺拼团活动id",required = true)
    @NotNull(message = "店铺拼团活动id不能为空")
     ...
     * 成团有效时间几（小时）
     */
    @ApiModelProperty(value = "成团有效时间几（小时）",required = true)
    @NotNull(message = "成团有效时间几（小时）不能为空")
    ...
    @Valid
    private List<CereShopGroupWorkDetail> details;
